...
const allFreeLaptops = asyncHandler( async (req, res) => {
    ...
    .status(200)
    .json(
        new ApiResponse(201, freeLaptops, "All Free laptops fetched")
    )
})
...
const newReturn = asyncHandler(async (req, res) => {
    ...
    if(!updateLaptop){
        throw new ApiError(500, "An error occured while updating laptop condition")
    }
...
const allReturn = asyncHandler( async (req, res) =>{
    ...
    .status(200)
    .json(
        new ApiResponse(201, allReturnedLaptops, "Successful fetching allReturnedLaptops")
    )
