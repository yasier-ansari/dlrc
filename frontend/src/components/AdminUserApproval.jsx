...
const AdminUserApprovalComp = ({ flag, id }) => {
    ...
    useEffect(() => {
        const getUserInfo = async () => {
            if (userType === 'admin') {
                const response = await fetch(`http://localhost:8000/api/v1/admin/request/${id}`, {
                    method: "GET",
                    credentials: "include",
        ...
        <>
            {
                !userNotFound ? <>
                    <div className="flex flex-col items-center justfiy-center max-w-4xl w-full h-full">
                        <div div className="flex items-center space-x-6 text-3xl sm:text-4xl md:text-5xl lg:text-6xl  justify-center w-full h-full mt-12 mb-16" >
                            <LuUserCircle2 className=" text-[#40916c] " />
                            <h1 className="font-bold  italic" >Application</h1>
                        </div>

                    </div>
                    <div className="flex items-center w-full justify-center mx-auto flex-col space-y-4 sm:space-y-6 md:space-y-12 max-w-6xl  mb-20" >
                        {/* <p>
                        ...
                        <div className="w-full flex flex-col md:flex-row items-center justify-between space-y-6 md:space-y-0 md:space-x-6  " >
                            <button onClick={(e) => { setModalPopped("https://external-content.duckduckgo.com/iu/?u=https%3A%2F%2Fgraphicsfamily.com%2Fwp-content%2Fuploads%2F2020%2F07%2FFree-online-ID-card-Template--2048x1152.jpg&f=1&nofb=1&ipt=f3f4332deb3ace7f8c6fb38df44ff2ff561dfeb90bcfd202d9c1e0434908c6bf&ipo=images"); }} className="flex flex-col items-start justify-center space-y-2 basis-[60%] w-full " >
                                ...
                                {/* <p className="font-bold text-sm sm:text-base md:text-lg pl-4 rounded-lg py-2 px-3 sm:px-4 md:px-6 border-2 w-full bg-green-100/20 border-zinc-300 "  >Yasier Zahir Ansari</p> */}
