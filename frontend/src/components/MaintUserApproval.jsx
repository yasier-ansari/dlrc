	...
	useEffect(() => {
		const getUserInfo = async () => {
			...
			try {
				...
				response = await axios({
					method: 'get',
					url: `http://localhost:8000/api/v1/admin/approved-request/${id}`,
					headers: {
						Authorization: `Bearer ${token}`
					}
				})
...
'
							...
							<div className='w-full flex flex-col md:flex-row items-center justify-between space-y-4 md:space-y-0 md:space-x-6'>
								<div className='flex flex-col items-start justify-center space-y-2 basis-[60%] w-full'>
									...
									<p className='truncate truncate font-medium outline-[#40916c] text-[0.8rem] sm:text-base md:text-lg pl-4 rounded-lg py-2 px-3 sm:px-4 md:px-6  w-full bg-stone-200'>
										Yasier Zahir Ansari
									</p>
								</div>
